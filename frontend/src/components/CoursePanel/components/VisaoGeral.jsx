...
const VisaoGeral = ({ visaoGeralData }) => {
  ...
  const dataNotaGeral = [
    { name: 'Nota Geral',
      'Curso': visaoGeralData.nota_geral_media ?? 0,
      'UFC': visaoGeralData.media_ufc_geral ?? 0,
      'Nacional': visaoGeralData.media_nacional_geral ?? 0,
    }
  ];
  const dataFormacaoGeral = [
    { name: 'Form. Geral',
      'Curso': visaoGeralData.nota_fg_media ?? 0,
      'UFC': visaoGeralData.media_ufc_fg ?? 0,
      'Nacional': visaoGeralData.media_nacional_fg ?? 0,
    }
  ];
  const dataCompEspecifico = [
    { name: 'Comp. Específico',
      'Curso': visaoGeralData.nota_ce_media ?? 0,
      'UFC': visaoGeralData.media_ufc_ce ?? 0,
      'Nacional': visaoGeralData.media_nacional_ce ?? 0,
    }
  ];
    ...
    <div className="space-y-10">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-center">
        <div className="bg-blue-100 p-4 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold text-blue-900">{visaoGeralData.nota_geral_media ?? 'N/A'}</p>
        </div>
        <div className="bg-green-100 p-4 rounded-lg shadow">
          <p className="text-sm text-green-700 font-semibold">Nota Formação Geral</p>
          <p className="text-3xl font-bold text-green-900">{visaoGeralData.nota_fg_media ?? 'N/A'}</p>
        </div>
        <div className="bg-yellow-100 p-4 rounded-lg shadow">
          <p className="text-sm text-yellow-700 font-semibold">Nota Comp. Específico</p>
          <p className="text-3xl font-bold text-yellow-900">{visaoGeralData.nota_ce_media ?? 'N/A'}</p>
        </div>
        ...
        <div style={{ width: '100%', height: 300 }}>
          ...
          <ResponsiveContainer>
            <BarChart data={dataNotaGeral} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Tooltip />
              <Bar dataKey="Curso" fill="#4F46E5" radius={[0, 10, 10, 0]}>
                <LabelList dataKey="Curso" position="right" formatter={(value) => value?.toFixed(1)} />
              </Bar>
              <Bar dataKey="UFC" fill="#10B981" radius={[0, 10, 10, 0]}>
                <LabelList dataKey="UFC" position="right" formatter={(value) => value?.toFixed(1)} />
              </Bar>
              <Bar dataKey="Nacional" fill="#FBBF24" radius={[0, 10, 10, 0]}>
                <LabelList dataKey="Nacional" position="right" formatter={(value) => value?.toFixed(1)} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
