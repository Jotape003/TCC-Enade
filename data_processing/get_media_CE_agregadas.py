...
from config import PROCESSED_DATA_PATH, YEARS_TO_PROCESS, FINAL_CE_JSON_PATH, FINAL_MEDIA_JSON_PATH # Ajustado para FINAL_CE_JSON_PATH
...
# --- Caminhos para os arquivos de médias PRÉ-CALCULADOS ---
MEDIAS_NACIONAIS_BASE_PATH = os.path.join(FINAL_MEDIA_JSON_PATH, 'Desempenho_Topico/Medias_Nacionais_Competencia')
MEDIAS_REGIAO_BASE_PATH = os.path.join(FINAL_MEDIA_JSON_PATH, 'Desempenho_Topico/Medias_Regiao_Competencia')
MEDIAS_UF_BASE_PATH = os.path.join(FINAL_MEDIA_JSON_PATH, 'Desempenho_Topico/Medias_UF_Competencia')
# (Não precisamos de um caminho base para médias UFC, pois este script irá CALCULAR)
...
def load_averages_all_years(base_path, file_name_pattern, average_type_name):
    """Carregador genérico para estruturas de médias anuais (Nacional, Região, UF)."""
    print(f"Carregando médias {average_type_name} de '{base_path}'...")
    averages_map = {}
    ...
    for year in YEARS_TO_PROCESS:
        file_path = os.path.join(base_path, str(year), file_name_pattern)
        
        if os.path.exists(file_path):
            data_year = load_json(file_path, f"{average_type_name} {year}")
            if data_year:
                 averages_map[str(year)] = data_year
        else:
             print(f"  -> Aviso: Arquivo de médias {average_type_name} para {year} não encontrado em {file_path}.")
             averages_map[str(year)] = {}
             
    print(f"Médias {average_type_name} carregadas.")
    return averages_map

# --- Passagem 1: Pré-calcular Médias UFC (Sem alteração) ---
# (Exatamente como no seu script)
...
def analisar_e_salvar_dados_por_curso(map_competencias, curso_grupo_map, ufc_avg_map, nacional_avg_map, regiao_avg_map, uf_avg_map):
    print("\n--- INICIANDO PASSAGEM 2: Calculando Médias por Curso e Juntando com Médias UFC, UF, Região e Nacionais ---")
            ...
            try:
                ...
                for curso_id_int, comps in results_por_curso.items():
                    ...
                    for comp_nome, data in comps.items():
                        ...
                        media_nacional_comp = nacional_avg_map.get(str(year), {}).get(co_grupo_str, {}).get(comp_nome, {})
                        media_regiao_comp = regiao_avg_map.get(str(year), {}).get(co_grupo_str, {}).get(comp_nome, {})
                        media_uf_comp = uf_avg_map.get(str(year), {}).get(co_grupo_str, {}).get(comp_nome, {})
                        ...
                        final_results_campus[curso_id_str].append({
                            ...
                            # Região
                            "percentual_objetivas_regiao": media_regiao_comp.get('percentual_objetivas_regiao'),
                            "media_discursivas_regiao": media_regiao_comp.get('media_discursivas_regiao'),
                            "n_objetivas_validas_regiao": media_regiao_comp.get('n_objetivas_validas_regiao'),
                            "n_discursivas_validas_regiao": media_regiao_comp.get('n_discursivas_validas_regiao'),
                            # UF
                            "percentual_objetivas_uf": media_uf_comp.get('percentual_objetivas_uf'),
                            "media_discursivas_uf": media_uf_comp.get('media_discursivas_uf'),
                            "n_objetivas_validas_uf": media_uf_comp.get('n_objetivas_validas_uf'),
                            "n_discursivas_validas_uf": media_uf_comp.get('n_discursivas_validas_uf')
                        })
                    final_results_campus[curso_id_str].sort(key=lambda x: x['competencia'])

                if final_results_campus:
                    output_dir = os.path.join(FINAL_CE_JSON_PATH, campus_name) # Use FINAL_CE_JSON_PATH
                    ...
                    print(f"  -> Sucesso! Análise de competência COM TODAS AS MÉDIAS salva em '{output_path}'")
...
def main():
    ...
    # Carrega todos os mapas de médias pré-calculados
    nacional_avg_map = load_averages_all_years(MEDIAS_NACIONAIS_BASE_PATH, "medias_nacionais_competencia.json", "Nacionais")
    regiao_avg_map = load_averages_all_years(MEDIAS_REGIAO_BASE_PATH, "medias_regiao_competencia.json", "Região")
    uf_avg_map = load_averages_all_years(MEDIAS_UF_BASE_PATH, "medias_uf_competencia.json", "UF")

    if not all([map_competencias, curso_grupo_map, nacional_avg_map, regiao_avg_map, uf_avg_map]):
        print("Encerrando script devido a erro no carregamento de um ou mais arquivos de mapeamento/médias.")
        ...
        print(f"  Mapa Região: {'OK' if regiao_avg_map else 'FALHOU'}")
        print(f"  Mapa UF: {'OK' if uf_avg_map else 'FALHOU'}")
        return
    ...
    if ufc_avg_map:
        analisar_e_salvar_dados_por_curso(map_competencias, curso_grupo_map, ufc_avg_map, nacional_avg_map, regiao_avg_map, uf_avg_map)
    else:
