...
def main():
    ...
    for year in YEARS_TO_PROCESS:
        ...
        medias_ce_ano, medias_fg_ano = calculate_ne_averages_competencia(year, year_extract_path, relevant_grupos, map_competencias_ce, map_competencias_fg) 
        ...
        if medias_ce_ano:
            ...
            try:
                ...
                print(f"   -> Médias CE (NE) de {year} salvas em '{output_path_ce}'")
            except Exception as e:
                print(f"   -> ERRO ao salvar médias CE (NE) de {year}: {e}")
        else:
        ...
        if medias_fg_ano:
            ...
            try:
                ...
                print(f"   -> Médias FG (NE) de {year} salvas em '{output_path_fg}'")
            except Exception as e:
                print(f"   -> ERRO ao salvar médias FG (NE) de {year}: {e}")
        else:
            print(f"   -> Aviso: Não foram calculadas médias FG (NE) para {year}.")

    print("\nProcesso de geração de médias da Região Nordeste por ano concluído.")
