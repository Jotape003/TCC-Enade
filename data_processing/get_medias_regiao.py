...
MEDIAS_REGIAO_BASE_PATH = os.path.join(FINAL_JSON_PATH, 'Medias_Regiao_Competencia')
...
def calculate_regional_averages(year, year_path, relevant_grupos, map_competencias):
    ...
    print(f"\nCalculando médias da REGIÃO (Nordeste) para {year} (chunks)...")
    ...
    info_required = {
        ...
        'CO_REGIAO_CURSO': ['CO_REGIAO_CURSO', '"CO_REGIAO_CURSO"'] # <-- FILTRO NOVO
    }
    ...
    for file in all_raw_files:
        if not info_file_path:
            ...
            if found_map:
                ...
                print(f"    -> Arquivo de Info (Região) encontrado: {os.path.basename(info_file_path)}")
    ...
    real_info_col_regiao = info_cols_map['CO_REGIAO_CURSO'] # <-- FILTRO NOVO
    ...
    try:
        ...
        df_info = pd.read_csv(info_file_path, sep=';', encoding='latin1', low_memory=False,
                              usecols=[real_info_col_curso, real_info_col_grupo, real_info_col_regiao]) # <-- Adiciona Região
        
        # Converte tipos
        df_info[real_info_col_regiao] = pd.to_numeric(df_info[real_info_col_regiao], errors='coerce') # <-- FILTRO NOVO
        ...
        df_info.dropna(subset=[real_info_col_curso, real_info_col_grupo, real_info_col_regiao], inplace=True)
        
        # Converte para int
        df_info[real_info_col_regiao] = df_info[real_info_col_regiao].astype(int) # <-- FILTRO NOVO
        ...
        df_info_regiao = df_info[df_info[real_info_col_regiao] == 2] 
        ...
        if not relevant_cursos:
             print(f"  -> Aviso: Nenhum curso relevante encontrado para Região 2 em {year}.")
             return None
        ...
        print(f"  -> Lendo {os.path.basename(notas_file_path)} em chunks (filtrando para {len(relevant_cursos)} cursos da Região)...")
        reader = pd.read_csv(
            notas_file_path, sep=';', encoding='latin1', low_memory=False, 
            usecols=cols_to_read_notas, iterator=True, chunksize=chunk_size
        )

        for chunk in tqdm(reader, desc=f"Processando Chunks {year} Região"):
            # ... (Toda a lógica de processamento de chunk é IDÊNTICA ao script do Colab) ...
            # ... (Ela já filtra por relevant_cursos, que agora são os cursos da Região Nordeste) ...
        ...
        for grupo, comps in results_agg.items():
            ...
            for comp, data in comps.items():
                ...
                final_means[grupo][comp] = {
                    ...
                    "n_objetivas_validas_regiao": data['obj_validas'],
                    "n_discursivas_validas_regiao": data['disc_cont']
                }
        
        print(f"  -> Médias da Região (chunks) calculadas para {year}.")
        return final_means # Retorna { "4003": { "Algoritmos": {...}, ... }, ... }

    except Exception as e:
        print(f"  -> ERRO GERAL ao processar médias da Região de {year}: {e}")
        return None
...
def main():
    ...
    os.makedirs(MEDIAS_REGIAO_BASE_PATH, exist_ok=True)
    print(f"\nSalvando médias da Região em subpastas anuais dentro de '{MEDIAS_REGIAO_BASE_PATH}'...")
    ...
    for year in YEARS_TO_PROCESS:
        ...
        if medias_ano:
            ...
            year_dir = os.path.join(MEDIAS_REGIAO_BASE_PATH, str(year))
            os.makedirs(year_dir, exist_ok=True)
            output_path = os.path.join(year_dir, 'medias_regiao_competencia.json') # Nome do arquivo
    ...
    print("\nProcesso de geração de médias da Região por ano concluído.")
